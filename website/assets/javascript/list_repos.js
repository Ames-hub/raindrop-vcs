...
function list_containers() {
    ...
    function fetchData() {
        fetch(`${api_url}/api/vcs/repositories/list_all`, {
            ...
            .then(data => {
                // Data format: {'private': {repo_name: repo_desc}, 'public': {repo_name: repo_desc}}
                ...
                Object.keys(private_repos).forEach(repo_name => {
                    const repo_desc = private_repos[repo_name];
                    add_repo_to_list(repo_name, repo_desc);
                });
                Object.keys(public_repos).forEach(repo_name => {
                    const repo_desc = public_repos[repo_name];
                    add_repo_to_list(repo_name, repo_desc);
                });
            })
            .catch(error => {
